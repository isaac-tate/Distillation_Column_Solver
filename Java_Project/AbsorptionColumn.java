...
public class AbsorptionColumn{
  ...
  public AbsorptionColumn(Packing packing, Fluid fluid, double[] conditions){
    ...
    double zdiff = recalculateHeightDifference(conditions[2]);
  ...
  public AbsorptionColumn(AbsorptionColumn source){
    ...
    this.fluid = new Fluid(source.fluid); //Needs deep copy
  ...
  public double recalculateHeightDifference(double l){//use to recalculate the difference in heights for optimization
    ...
    for (int i=0;i<this.iterations;i++){
      ...
      //Calculating k'xa
      data[6][i] = Math.pow(((this.crossArea/data[2][i]) * (0.357/this.packing.fpPacking) * Math.pow( (this.fluid.getNsc_L()/372), 0.5) * Math.pow((data[4][i]/this.fluid.getMu_L())/(6.782/0.0008937), 0.3)), -1);
      //Calculating k'ya
      data[7][i] = Math.pow(((this.crossArea/data[3][i]) * (0.226/this.packing.fpPacking) * Math.pow((this.fluid.getNsc_V()/0.660), 0.5) * Math.pow((data[4][i]/6.782), -0.5) * Math.pow((data[5][i]/0.678), 0.35)), -1);
